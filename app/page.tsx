...
export default function Home() {
  return (
    <main>
      ...
      <section className={`${wrap} flex min-h-142.5 flex-col justify-between py-14 md:min-h-170 md:py-20`}>
        <p className="m-0 text-[11px] font-extrabold tracking-[0.14em] text-(--primary)">HCI / CREATIVE TECHNOLOGY / TAIPEI</p>
        <h1 className="my-10 max-w-295 text-[clamp(51px,8.6vw,124px)] font-semibold leading-[0.98] tracking-[-0.075em]">
          把數位邏輯，
        ...
        <div className="flex items-end justify-between gap-10">
          ...
          <a className="hidden size-16.5 shrink-0 place-items-center rounded-full border border-(--ink) text-2xl transition hover:translate-y-1 hover:bg-(--ink) hover:text-white sm:grid" href="#work" aria-label="前往作品區">↓</a>
        </div>
      </section>
      ...
      <section className={`${wrap} py-24 md:py-32`} id="work">
        <div className="mb-16 grid items-end gap-6 md:grid-cols-[1fr_2fr]">
          <p className="m-0 text-[11px] font-extrabold tracking-[0.14em] text-(--primary)">SELECTED WORK / 01—05</p>
        ...
        <div className="grid grid-cols-1 gap-x-8 gap-y-20 md:grid-cols-2" aria-label="精選作品">
          {projects.map((project) => (
            <Link key={project.index} href={project.href} className="group flex flex-col border-t border-(--ink) pt-3.5">
              <div className="relative aspect-3/2 overflow-hidden bg-(--soft)">
                <img className="size-full object-cover transition duration-500 group-hover:scale-102.5" src={project.image} alt={project.imageAlt} />
              ...
              <div className="flex flex-1 flex-col pt-5">
                ...
                <div className="mt-auto flex items-end justify-between gap-6 border-t border-(--line) pt-3.5">
