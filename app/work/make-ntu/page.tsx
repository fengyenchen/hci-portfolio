...
import { CaseHeader, CaseSection, NextProject, PrincipleGrid } from "../../components/CaseElements";
  ...
  return <main>
    ...
    <article>
      <CaseHeader compact label="MakeNTU 2026 / Hardware Hackathon" title={<>多模組化<br />救災探勘載具</>} subtitle="在不讓救災人員進入高風險區域的情況下，完成感知、判讀與任務操作。" meta={[
        { label: "我的角色", value: "IoT 監控、車體與模組電控、千斤頂小車結構" },
        { label: "技術面向", value: "感測資料、即時監控、模組化硬體" },
        { label: "成果", value: "企業獎第三名" },
      ]} />

      <figure className="m-0 w-full bg-white px-3 py-6 md:px-[5vw] md:py-16"><img className="mx-auto max-h-207.5 w-full object-contain" src="/assets/make-ntu.jpg" alt="多模組化救災探勘載具完整專案介紹" /></figure>
      ...
      <PrincipleGrid orange items={[
        { title: "感知", text: "硬體感測環境變數，降低人員進入危險區域的需要。" },
        { title: "解讀", text: "透過監控介面把原始數值轉成直觀的預警資訊。" },
        { title: "執行", text: "依據現場決策驅動千斤頂、瓦斯偵測等實體模組。" },
      ]} />

      <section className="mx-auto grid w-full max-w-310 gap-8 border-b border-(--ink) bg-white px-4 py-20 md:grid-cols-[1fr_2fr] md:gap-14 md:px-8 md:py-32">
        <p className="mt-2 text-[10px] font-extrabold tracking-[0.13em] text-(--green)">02 / MY CONTRIBUTION</p>
        <div>
          ...
          <ul className="mt-14 list-none p-0">
            ...
            ].map((item) => <li className="grid grid-cols-[55px_1fr] border-t border-(--line) py-5" key={item[0]}><span className="text-[11px] text-(--green)">{item[0]}</span><div><b className="text-lg">{item[1]}</b><p className="mt-1 text-sm text-(--muted)">{item[2]}</p></div></li>)}
          </ul>
        </div>
      </section>
      ...
      <CaseSection index="03 / TAKEAWAY" title={<>快速原型的速度，<br />來自清楚的系統邊界。</>}>
        ...
        <a className="mt-7 inline-block text-sm font-extrabold underline-offset-4 hover:underline" href="https://github.com/fengyenchen/MakeNTU2026" target="_blank" rel="noreferrer">查看 GitHub 專案 ↗</a>
      </CaseSection>

      <NextProject label="回到精選作品" href="/#work" title="所有作品" />
    </article>
    <SiteFooter />
  </main>;
}
