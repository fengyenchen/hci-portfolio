...
export function CreativeCase({ project }: { project: CreativeProject }) {
  return <main>
    ...
    <article>
      ...
      <section className={`${wrap} grid gap-8 border-b border-[var(--ink)] py-20 md:grid-cols-[1fr_2fr] md:gap-14 md:py-32`}>
        ...
        <div>
          ...
          <p className="mb-12 text-[15px] leading-7 text-[var(--muted)]">從操作流程、回饋方式到視覺節奏，逐步把技術能力轉化為使用者能理解的體驗。</p>
          <ol className="m-0 list-none border-t border-[var(--ink)] p-0">
            {project.focus.map((item, index) => <li className="grid grid-cols-[60px_1fr] gap-4 border-b border-[var(--line)] py-6" key={item}><span className={`text-[11px] font-extrabold ${project.accent}`}>0{index + 1}</span><p className="text-[17px] leading-[1.7]">{item}</p></li>)}
          </ol>
        </div>
      </section>
