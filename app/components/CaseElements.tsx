...
export const eyebrow = "text-[11px] font-extrabold uppercase tracking-[0.14em] text-(--green)";
...
export function CaseHeader({ label, title, subtitle, meta, children, compact = false }: { label: string; title: ReactNode; subtitle: string; meta?: { label: string; value: string }[]; children?: ReactNode; compact?: boolean }) {
  return (
    <header className={`${wrap} py-10 md:py-14`}>
      <Link className="mb-12 inline-block text-[13px] font-bold hover:text-(--green) md:mb-14" href="/#work">← 所有作品</Link>
      ...
      {meta && <dl className="mt-10 grid border-t border-(--ink) md:grid-cols-3">
        {meta.map((item) => <div className="border-b border-(--line) py-4 md:border-b-0 md:border-r md:px-7 md:first:pl-0" key={item.label}><dt className="text-[10px] font-extrabold tracking-[0.1em] text-(--green)">{item.label}</dt><dd className="mt-2 text-sm leading-6">{item.value}</dd></div>)}
      </dl>}
    </header>
  );
}
...
export function CaseSection({ index, title, children }: { index: string; title: ReactNode; children: ReactNode }) {
  return (
    <section className={`${wrap} grid gap-8 border-b border-(--ink) bg-white py-20 md:grid-cols-[1fr_2fr] md:gap-14 md:py-32`}>
      <p className="mt-2 text-[10px] font-extrabold tracking-[0.13em] text-(--green)">{index}</p>
      <div>
...
export function PrincipleGrid({ items, orange = false }: { items: { title: string; text: string }[]; orange?: boolean }) {
  return <section className={`${wrap} grid border-b border-(--ink) md:grid-cols-3`}>{items.map((item, index) => <div className="flex min-h-61.25 flex-col border-b border-(--ink) p-8 md:min-h-82.5 md:border-r md:first:border-l" key={item.title}><span className={`text-[11px] font-extrabold ${orange ? "text-(--orange)" : "text-(--green)"}`}>0{index + 1}</span><h3 className="mb-3 mt-auto text-[35px] font-semibold tracking-[-0.045em]">{item.title}</h3><p className="leading-[1.65] text-(--muted)">{item.text}</p></div>)}</section>;
}
